...
`wip worktree list` SHALL discover worktrees by reading the `worktrees/` directory at the repo root. For each subdirectory (treated as a submodule name), it SHALL enumerate its subdirectories (treated as worktree names).
...
- **WHEN** `worktrees/<submodule>/` exists but contains no subdirectories
- **THEN** that submodule produces no output rows

### Requirement: Output is a flat two-column table sorted alphabetically
The CLI SHALL print one row per worktree in the format `<submodule>  <worktree>` (two spaces as separator), sorted first by submodule name then by worktree name.

#### Scenario: Single worktree
- **WHEN** `worktrees/my-lib/feature-x/` exists
- **THEN** the CLI prints `my-lib  feature-x`

#### Scenario: Multiple worktrees across submodules
- **WHEN** worktrees exist for multiple submodules
- **THEN** the CLI prints all rows sorted by submodule name, then worktree name within each submodule

#### Scenario: Multiple worktrees under one submodule
- **WHEN** `worktrees/my-lib/feature-x/` and `worktrees/my-lib/bugfix-y/` both exist
- **THEN** the CLI prints both rows sorted alphabetically: `my-lib  bugfix-y` then `my-lib  feature-x`
