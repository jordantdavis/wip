...
Running `wip init` multiple times from the same git repo root SHALL produce the same result as running it once. Subsequent runs SHALL NOT fail, overwrite state, or produce spurious output.

#### Scenario: Repeated invocation
- **WHEN** the user runs `wip init` twice from the same git repo root
- **THEN** both invocations exit with code 0 and the second produces no output
