...
`wip worktree launch` SHALL read the `on-worktree-launch` list for the target ref from `.wip.yml` and execute each command in order inside the worktree directory. Unlike `on-worktree-create`, these hooks are expected to run every time the user launches a worktree and SHOULD be idempotent.

#### Scenario: on-worktree-launch hook executes successfully
- **WHEN** the ref has an `on-worktree-launch` list in `.wip.yml` and all commands exit with code 0
- **THEN** all commands run in order inside the worktree directory and the CLI exits with code 0

#### Scenario: No on-worktree-launch hook configured
- **WHEN** the ref has no `on-worktree-launch` entry in `.wip.yml`
...
Each command in the `on-worktree-launch` list SHALL be executed with its working directory set to the absolute path of the worktree (`<repo root>/worktrees/<ref>/<worktree>/`).
