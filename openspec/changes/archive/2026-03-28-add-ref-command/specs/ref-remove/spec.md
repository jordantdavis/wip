...
- **WHEN** the user runs `wip ref remove api`
- **THEN** the command proceeds with `api` as the ref name
...
- **THEN** the CLI runs deinit, then git rm, then rm -rf in that order

#### Scenario: Output is streamed
- **WHEN** a git command produces output during removal
- **THEN** that output is written to the terminal as it is produced
