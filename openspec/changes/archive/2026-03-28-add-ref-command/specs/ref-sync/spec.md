...
`wip ref sync` SHALL NOT stream per-ref output to the terminal during execution. After all operations finish, the CLI SHALL report one result line per ref: `✓ <name>` for success or `✗ <name>: <error>` for failure.
...
`wip ref sync` SHALL exit with code 0 only if every update succeeded. If one or more failed, it SHALL exit with a non-zero code.

#### Scenario: All succeed — exit code 0
- **WHEN** every ref update returns success
- **THEN** the CLI exits with code 0

#### Scenario: Any failure — non-zero exit code
- **WHEN** at least one ref update fails
