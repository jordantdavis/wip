...
func worktreeRemove(args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "usage: wip worktree remove [--delete-branch] <ref> <worktree>")
	}
	fs.Parse(args)
	...
	exists, err := refExists(submodule)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
	...
	if _, err := os.Stat(absWorktreePath); os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, "worktree %q not found in ref %q\n", worktree, submodule)
		os.Exit(1)
	}
